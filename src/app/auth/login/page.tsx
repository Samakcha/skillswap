...
import { Layers, Eye, EyeOff, Loader2, AlertCircle, ArrowLeft, CheckCircle, Mail, Lock, ArrowRight } from 'lucide-react'
            ...
            >
              ...
              <form onSubmit={handleLogin} className="space-y-4">
                ...
                <AnimatePresence mode="wait">
                  {signupSuccess && (
                    ...
                    >
                      <CheckCircle className="w-4.5 h-4.5 text-emerald-400 shrink-0 mt-0.5" />
                      <div>
                        <span className="block font-bold text-white text-[11px]">ACCOUNT CREATED SUCCESS!</span>
                        <span className="block text-[9px] text-emerald-400/80 mt-0.5">Please sign in with your credentials to get started.</span>
                      </div>
                    </motion.div>
                  )}
                </AnimatePresence>
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full mt-4 py-4.5 px-4 bg-[#FF4D00] text-black font-mono font-bold text-xs uppercase tracking-widest rounded-2xl border-2 border-black shadow-[4px_4px_0px_#FFFFFF] hover:shadow-none hover:translate-x-[2px] hover:translate-y-[2px] transition-all duration-200 active:scale-95 disabled:opacity-50 disabled:pointer-events-none cursor-pointer flex items-center justify-center gap-2 relative overflow-hidden"
                >
                  {loading ? (
                    <>
                      <Loader2 className="w-4.5 h-4.5 animate-spin text-black shrink-0" />
