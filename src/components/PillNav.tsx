...
const PillNav = ({
  ...
  useEffect(() => {
    const layout = () => {
      circleRefs.current.forEach(circle => {
        ...
        if (w === 0 || h === 0) return; // avoid math division by zero if hidden
  ...
  const renderLogo = () => {
    const content = typeof logo === 'string' ? (
      <img src={logo} alt={logoAlt} ref={logoImgRef} />
    ) : (
      logo
    );

    const targetHref = logoHref || items?.[0]?.href || '#';

    if (isRouterLink(targetHref)) {
      return (
        <Link
          className="pill-logo"
          href={targetHref}
